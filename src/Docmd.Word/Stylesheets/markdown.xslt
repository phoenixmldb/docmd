      ...
      <xsl:variable name="innermost" as="node()*">
        <xsl:for-each select="w:t | w:br | w:tab | w:noBreakHyphen | w:sym | w:drawing">
          <xsl:choose>
            ...
            <!--
              w:char is a code point in the font's own encoding, not Unicode, so this reads
              the character SymbolResolver worked out rather than deciding again here. A
              w:sym whose font is unknown carries no docmd:char and emits nothing, and the
              coverage check then reports it: omit and say so, never guess.
            -->
            <xsl:when test="self::w:sym"><md:text><xsl:value-of select="@docmd:char"/></md:text></xsl:when>
